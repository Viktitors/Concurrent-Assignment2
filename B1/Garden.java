...
class Counter {
    ...
    void increment(int ID) {
       //prepotocol
        wantsCS[ID]=true;  //if I want to enter and I do cs and post-protocol
        while(wantsCS[1-ID]){   //while the other wants to enter
            if(turn==1-ID){     //if it's its turn 
                wantsCS[ID]=false; //I dont want to enter
                while(turn==1-ID); //while its its turn 
                wantsCS[ID]=true;  //next turn
            }
        }
        //cs 
        ...
        //non-CS
        //post-protocol
        turn=1-ID;     // Now it's the other one's turn
        wantsCS[ID]=false;
    }
}
